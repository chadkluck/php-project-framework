 ...
 *
 * $x = &getApp("myvar") is the same as $x = &$app['myvar']
 * and then performing $x++ will update the 'myvar' value
 *
 ...
 * 
 * If you only want to obtain a copy of the value, but not update it
 * do not use the &
 * $v = getApp("myVal");
 * $v++ will not update "myVal" in app
 *
...
/**
 * Set the element at index of the global app variable to $value
 * 
 * @param String $index the index of the variable
 * @param Mixed $value the value you want to set it to
 * @return Mixed The new value
 */
function &setApp( $index = NULL, $value ) {
	global $app;

	if ( $index && isset( $app[$index])) {
		$app[$index] = $value;
	}
 ...
 */
function logMsg($message, $dataArray = NULL ) {
 ...
 */
function logAPIrequest($url, $response = NULL) {
 ...
 * @param array $dataArray Optional. Any associated array data to log
 */
function logError($message, $dataArray = NULL) {
 ...
 */
function logEntry($message, $type, $data = NULL ) {
...
function setExeStartTime() {
	...
	setApp("start_time", microtime(TRUE));
	logMsg("Starting app");
}

function setExeEndTime() {
	setApp("end_time", microtime(TRUE));
	setApp("exe_time", (getApp("end_time") - getApp("start_time")));
}
...
function getRequestOrigin() {
	...
	$appRequest = &getApp("request");

	if( !isset($appRequest['origin'])) {
		$o = "";
...
function authorizeAPIcall($die = true) {
    ...
    */
	...
	$keys = (getCfg("secrets") && isset(getCfg("secrets")['api-restrict-allow-key'])) ? getCfg("secrets")['api-restrict-allow-key'] : array();
	$ip =   (getCfg("security") && isset(getCfg("security")['api-restrict-allow-ip'])) ? getCfg("security")['api-restrict-allow-ip'] : "";
...
/*	--------------------------------------------------------------------------------------------
	proxifyURL()
	Modified 2019-12-19 - chadkluck
*/

/**
 * If cache proxy is set up this will proxify a URL according to the cacheproxy
 * configuration. If no cache proxy is set up then it will just return the URL 
 * sent to it.
 * 
 * This function is useful in code that wants to take advantage of cache proxy, 
 * but may not have it set up yet. It essentially makes your code cache proxy 
 * ready.
 * 
 * This is automactilly invoked in all of the getData and generateRequest 
 * functions. This should only be used when you are making requests that don't 
 * utilize getData and generateRequest functions. (For example when you use
 * simplexml_load_file() or curl directly)
 * 
 * See PHP Project Framework script file cache-proxy.php for more info about 
 * cache proxy.
 */
function proxifyURL($url) {
	// cacheProxy is a php-project-framework add on
	if ( getApp("obj") !== NULL && isset(getApp("obj")['x-cacheProxy']) ) {
		$url = getApp("obj")['x-cacheProxy']->getURI($url);
	}
...
function getData($url, $headers = NULL, $returnResponseHeaders = FALSE) {
	...
	// proxify the URL if we are sending via proxy
	$url = proxifyURL($url); // if no proxy, the same URL is sent back
